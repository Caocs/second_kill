...
//        }
    ...
    /**
     * 模拟一个更新User的操作，需要删除redis中的user缓存。
     */
    @Override
    public ResponseBean updatePassword(String userTicket, Long id, String password,
                                       HttpServletRequest request, HttpServletResponse response) {
        User user = getUserByCookie(userTicket, response);
        if (user == null) {
            throw new GlobalException(ResponseBeanEnum.LOGIN_ERROR);
        }
